<div class="flex flex-col h-full bg-white rounded-2xl shadow-md overflow-hidden product-card {{ $class ?? '' }} relative transition-shadow duration-300 hover:shadow-lg {{ $product->offer ? 'ring-2 ring-[#FF6B35]/30' : '' }}">
    ...
    <div class="h-52 md:h-64 bg-[#F5F5F5] flex flex-shrink-0 items-center justify-center overflow-hidden rounded-t-2xl">
        ...
        <div class="mb-6">
            @if($product->offer)
                <div class="flex items-baseline gap-3">
                    <span class="text-base text-[#1A1C1E]/50 line-through">€{{ number_format($product->price, 2) }}</span>
                    <span class="text-3xl md:text-4xl font-bold text-[#006D77]">€{{ number_format($product->final_price, 2) }}</span>
                </div>
            @else
                <span class="text-3xl md:text-4xl font-bold text-[#006D77]">€{{ number_format($product->final_price ?? $product->price, 2) }}</span>
            @endif
        </div>
